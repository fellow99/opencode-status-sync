...
| OpenCode 扩展点 | 触发机制 | status | API 接口 |
|----------------|---------|--------|---------|
| `session.created` / `session.status` | `event` hook | `thinking` | `/thinking` |
| `session.idle` | `event` hook | `idle` | `/idle` |
| `session.error` | `event` hook | `error` | `/error` |
| `tool.read` / `glob` / `grep` | `tool.execute.before` | `reading` | `/reading` |
| `tool.edit` / `write` | `tool.execute.before` | `writing` | `/writing` |
| `tool.bash` / 其他工具 | `tool.execute.before` | `working` | `/working` |
| 任意工具执行完毕（无错误） | `tool.execute.after` | `thinking` | `/thinking` |
...
| `mapping[].status` | 是 | — | **OpenCode 扩展点**（状态、事件、调用的逻辑名称） |
