...
**Goal**: Rename all project identifiers from `opencode-pets` to `opencode-status-sync`.

- [ ] T001 [P] Rename config file `opencode-pets.json` → `opencode-status-sync.json` with new schema format (debug, baseURL, headers, mapping with 6 endpoints)
- [ ] T002 [P] Update `package.json`: change name to `opencode-status-sync`, update description and keywords
- [ ] T003 [P] Update `README.md`: replace all `opencode-pets` references with `opencode-status-sync`, update config format documentation
...
- [ ] T006 [US1] Refactor plugin entry in `.opencode/plugins/opencode-status-sync.ts` — rename from `OpenCodePetsPlugin` to `OpenCodeStatusSync`, update all log service names from `opencode-pets` to `opencode-status-sync`, update all console prefixes from `[🐱 pets]` to `[📡 status-sync]`
...
- [ ] T011 [P] Update `opencode.json` in project root — remove any `opencode-pets` references if present
- [ ] T012 [P] Update `specs/constitution.md` — change project name from `opencode-pets` to `opencode-status-sync`
...
- [ ] T019 Final verification — build passes, all old `opencode-pets` references removed, config file renamed
